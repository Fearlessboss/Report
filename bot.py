#!/usr/bin/env python3
"""
ILLEGAL CONTENT DETECTOR BOT
Owner + Sudo | Powered by AI + Telethon

FIXES v7 (inline buttons not responding):
  * Removed INVALID style= kwarg from InlineKeyboardButton
    (python-telegram-bot does NOT support style= - it silently breaks
    keyboard creation / crashes callbacks)
  * CallbackQueryHandler(cb_router) added to entry_points too, so buttons
    keep working even after conversation_timeout expires
  * run_polling(..., allowed_updates=Update.ALL_TYPES) so callback_query
    updates are always received
  * concurrent_updates(False) retained (no ConversationHandler races)
  * All secrets from environment variables
  * query.answer() always called first, full callback logging
...
# ══════════════════════════════════════════════════════════
# API KEYS (environment variable se load hota hai)
# ══════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════
# KEYBOARDS  (style= kwarg REMOVED - not supported by PTB)
# ══════════════════════════════════════════════════════════
...
async def analyze_illegality(messages: List[str], ch_info: dict) -> dict:
    # Truncate each message to 300 chars to stay under token limit
...
async def generate_send_targets(analysis: dict, ch_info: dict) -> str:
    ...
    sys_prompt = (
        ...
        "• One email per line, format: <email> — <why>\n"
        "• Always include abuse@telegram.org first\n"
        "• Include dmca@telegram.org if copyright is involved\n"
        "• Include child-abuse hotlines (e.g. CyberTipline at NCMEC) if child abuse is involved\n"
        "• Include relevant national cyber-crime reporting portals if severity is CRITICAL/HIGH\n"
        "• Include GDPR/data-protection authority if personal data violation\n"
...
async def ub_join_and_fetch(
    ...
    try:
        ...
        async for msg in userbot_client.iter_messages(entity, **iter_kwargs):
            ...
            # Photo messages -> download for vision AI
            if isinstance(msg.media, MessageMediaPhoto) and images_downloaded < MAX_IMAGES:
                try:
                    img_bytes = await userbot_client.download_media(msg, file=bytes)
                    if img_bytes:
                        image_list.append({
                    ...
                    logger.warning(f"Image download failed (msg {msg.id}): {dl_ex}")

    except Exception as ex:
        logger.exception("iter_messages failed")
        raise RuntimeError(f"Could not fetch messages: {ex}")

    return entity, ch_info, messages, msg_links, image_list


async def _resolve_target(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Tuple[Optional[int], Optional[str], Optional[str]]:
...
async def cb_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Always answer the callback first (removes the "loading" spinner)
    try:
        await query.answer()
...
async def hdl_verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Detect specific message link (e.g. t.me/channel/123) -> fetch from that point
    start_msg_id = 0
    ...
    if analysis.get("is_illegal") or illegal_image_results:
        ...
        violations_txt = "\n".join(f"• {e(v)}" for v in violations) if violations else "• See detailed reason"
        laws_txt       = "\n".join(f"• {e(l)}" for l in laws)       if laws       else "• See detailed reason"

        if illegal_links:
            links_section = (
                "\n\n🔗 Illegal Message Links (Direct Evidence):\n"
                + "\n".join(
                    f'  {i + 1}. <a href="{e(lnk)}">View Message {i + 1}</a>'
        ...
        # If only image violations exist, pick best category from image results
        ...
        report = (
            ...
            f"{uname_line}"
            f"• Type    : {e(str(ch_info.get('type', 'N/A')).upper())}\n"
...
async def hdl_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if illegal_links:
        ev_block = (
            ...
            + "\n".join(f"  {i + 1}. {e(lnk)}" for i, lnk in enumerate(illegal_links[:10]))
            + "\n"
        )
...
def main() -> None:
    ...
    app = (
        Application.builder()
        .token(BOT_TOKEN)
        .concurrent_updates(False)      # sequential updates, no ConversationHandler races
        .post_init(post_init)
        .build()
    )
    ...
    conv = ConversationHandler(
        # KEY FIX: CallbackQueryHandler in entry_points so buttons keep
        # working even after conversation_timeout expires (allow_reentry)
        entry_points=[
    ...
    # Global fallback: if for ANY reason the conversation did not catch a
    # callback, this answers it so the button never stays dead.
    async def global_cb_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    app.add_handler(CallbackQueryHandler(global_cb_fallback), group=1)
    ...
    # KEY FIX: allowed_updates ensures callback_query updates are received
    app.run_polling(
